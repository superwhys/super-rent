...
update_info_desc = 'POST请求， 参数: 用户名， update_info: {"update_info": [{"type": choice("user_name", "phone", "authority", "password") , "msg": ...}]}, 需在headers中携带登录后的token, 字段为Authorization'


"""tenant description"""
create_tenant_desc = 'POST请求, 参数: 租客名 出租单位(可选) 出租单元(可选) 租期状态(可选) 计划租房月数(可选) 电话 身份证, 需在headers中携带登录后的token, ' \
                     '字段为Authorization '
get_tenant_info_desc = "GET请求, 参数: 租客名 出租单位 出租单元, 需在headers中携带登录后的token, 字段为Authorization"
